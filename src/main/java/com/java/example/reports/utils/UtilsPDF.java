...
//			PDDocument mainDoc = PDDocument.load(new File(fileName)); 
			boolean isLandscape = isDocLandscape(fileName);
			...
			float curYVal = upperRightY;
			if (isLandscape) {
				curYVal = upperRightX - 50f;
			}
			...
			for (int idx = 0; idx < keys.length; idx++) {
				String headerText = keys[idx] + "  --------------------- ";
				Integer pageNo = keyToPageNoMap.get(keys[idx]);
				curYVal = curYVal - 20;
				contentStream.beginText();
				contentStream.newLineAtOffset(25, curYVal);
				contentStream.showText(headerText + pageNo);
				contentStream.endText();
...
//				float offset = (font.getStringWidth(headerText) / 1000) * 18;
//				float textWidth = (font.getStringWidth(String.valueOf(pageNo)) / 1000) * 18;
//				PDRectangle position = new PDRectangle();
//				position.setLowerLeftX(25);
//				position.setLowerLeftY(curYVal - 24f);
//				position.setUpperRightX(25 + offset + textWidth);
//				position.setUpperRightY(curYVal -4);
//				txtLink.setRectangle(position);
//				 
//				// add an action
//			    PDPageDestination destination = new PDPageFitWidthDestination();
//		        PDActionGoTo action           = new PDActionGoTo();
//		        destination.setPageNumber(pageNo);
//		        action.setDestination(destination);
//
////		        PDActionURI action = new PDActionURI();
////				action.setURI("http://www.pdfbox.org");
//				txtLink.setAction(action);
//				txtLink.setPage(page);
//				
//				// and that's all ;-)
...
//			contentStream.endText();
			...
			// build page links
			createTOCPageLinks(path2.toFile().getAbsolutePath());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	private static void createTOCPageLinks(String fileName) {
		try (PDDocument document = PDDocument.load(new File(fileName));) {
			...
			//
			PDRectangle position = page.getMediaBox();
			link.setContents("HELLL");
...
//				contentStream.setLeading(20f);
			...
			float curYVal = upperRightY;
			if (isDocLandscape(fileName)) {
				curYVal = position.getUpperRightX() - 50f;
			}
			System.out.println("HELLLL ==> " + page.getContents().toString());
			PDFTextStripper stripper = new MyPDFTextStripper();
			stripper.setStartPage(0);
			stripper.setEndPage(1);
			Writer dummy = new OutputStreamWriter(new ByteArrayOutputStream());
			stripper.writeText(document, dummy);
//			String text = stripper.getText(document);
//				System.out.println("HELLLLLLL ==> "+text);

			float offset = (font.getStringWidth("Total Asset Value ---------------------") / 1000) * 18;
			float textWidth = (font.getStringWidth(String.valueOf(3)) / 1000) * 18;
			position.setLowerLeftX(25 + offset);
			position.setLowerLeftY((curYVal - 20 - 20));
			position.setUpperRightX(25 + offset + textWidth);
			position.setUpperRightY(curYVal - 20);
			link.setRectangle(position);

//				contentStream.beginText();
//				contentStream.newLineAtOffset(210, curYVal);
//				contentStream.showText("HELLLLLLLLLL");
//				contentStream.endText();
//				contentStream.close();

			destination.setPage(document.getPage(3));
			action.setDestination(destination);
...
//			link.setRectangle(page.getMediaBox());
	...
	private static boolean isDocLandscape(final String fileName) throws IOException {
		try (PDDocument document = PDDocument.load(new File(fileName));) {
			if (document != null) {
